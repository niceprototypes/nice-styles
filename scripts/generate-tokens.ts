...
/**
 * Generate types.ts file with TypeScript type definitions
 *
 * Creates type definitions for each token group. Each type is a union
 * of all valid keys for that token.
 *
 * @param tokens - Array of parsed Token objects
 * @returns Complete TypeScript types file content as a string
 *
 * @example
 * // Given animationDuration token with keys "base" and "slow", generates:
 * export type AnimationDurationType = "base" | "slow"
 */
function generateTypesTSFile(tokens: Token[]): string {
  const lines: string[] = []

  // Add header comment
  lines.push(`/**`)
  lines.push(` * Type definitions for nice-styles design tokens`)
  lines.push(` *`)
  lines.push(` * This file contains TypeScript type definitions for all token groups.`)
  lines.push(` * Each type represents the valid keys for a specific token group.`)
  lines.push(` *`)
  lines.push(` * This file is auto-generated by scripts/generate-tokens.ts.`)
  ...
  for (const token of tokens) {
    ...
    const typeName = token.name.charAt(0).toUpperCase() + token.name.slice(1) + 'Type'

    // Create union of all keys
    const keys = token.entries.map(entry => `"${entry.key}"`).join(' | ')

    lines.push(`export type ${typeName} = ${keys}`)
    lines.push(``)
  }
 ...
 * 3. Generates src/types.ts with TypeScript type definitions
 * 4. Generates variables.css with all CSS custom properties
 * 5. Generates individual CSS files per token in dist/css/
 *
...
function main() {
  ...
  const typesTSPath = path.join(__dirname, '..', 'src', 'types.ts')
  ...
  // Generate src/types.ts with type definitions
  const typesTSContent = generateTypesTSFile(tokens)
  fs.writeFileSync(typesTSPath, typesTSContent, 'utf-8')
  console.log(`✓ Generated TypeScript types: ${typesTSPath}`)
