 ...
 * layers over it; a new name creates a runtime-only entry with the same
 * metadata seeding records (`writeToken`). Entries are never replaced
 * wholesale — the seed survives, and readers fall back to it for any theme or
 * breakpoint the override does not cover.
 *
...
export function registerTokens(
  ...
  for (const [group, variants] of Object.entries(tokenMap)) {
    for (const [variant, value] of Object.entries(variants)) {
      writeToken('runtime', { prefix, path: [group], variant }, value)
    }
  }
}
